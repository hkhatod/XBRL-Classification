...
class TextCNNRNN(object):
	def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
		...
		with tf.name_scope("Attention"):
			...
			self.scores1 = tf.layers.dense(attention_output, num_classes, activation=None)
		...
		with tf.name_scope('output'):
			...
			self.scores2 = tf.nn.xw_plus_b(self.output, W, b, name='Scores')
			
		with tf.name_scope('combined_scored'):
