...
def train_cnn_rnn():
	...
	if params['continue_training']:
		...
		checkpoint_dir = directory +'/'+ 'BiDRNN_CNN' + '/'
		''' i is a folder increment varaiable. Its also used to update the name of tsv file.'''
		i = 1
		if os.path.exists(checkpoint_dir):
			while os.path.exists(directory  +'/'+ 'BiDRNN_CNN' + str(i) + '/'):
				i += 1
				''' dont del i as emb_viz is using for incremnting '''
			checkpoint_dir = directory  +'/'+  'BiDRNN_CNN' + str(i) + '/' + runname + '/'
		else:
			''' This del is OK '''
			del i
			i = ''
			checkpoint_dir = directory  +'/'+ 'BiDRNN_CNN' + '/' + runname + '/'
