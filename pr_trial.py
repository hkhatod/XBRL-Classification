...
def start_runs(logdir, steps, run_name, thresholds, mask_every_other_prediction=False):
    ...
    true_reds = tf.clip_by_value(
        tf.concat([255 - tf.abs(channel_distribution.sample([number_of_reds, 1])),
    ...
    for i, color in enumerate(('red', 'green', 'blue')):
        ...
        if mask_every_other_prediction:
            ...
            consecutive_indices = tf.reshape(tf.range(tf.size(predictions[i])), tf.shape(predictions[i]))
            weights = tf.cast(consecutive_indices % 2, dtype=tf.float32)

        logging.warning(" labels : {}".format(labels[:,i]))
        logging.warning(" predictions : {}".format(predictions[i]))
        summary.op(
            color,
            labels=labels[:, i],
