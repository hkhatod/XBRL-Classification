	...
	def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
		...
		for i in range(num_classes):
			with tf.name_scope('%s' % self.text_labels[i]):
				self.pr_curve, self.update_op = summary_lib.pr_curve_streaming_op('pr_curve', predictions=self.probabilities[:,i], labels=tf.cast(self.input_y,tf.bool)[:,i], num_thresholds=200)
